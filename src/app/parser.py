...
class AgendaParser:
    ...
    def __init__(self, api) -> None:
        ...
        self.parser = argparse.ArgumentParser(
            description='Command-line parser for the Monica Scheduler distributed agenda')
    ...
    def _inbox(self):
        ...
        for req in user.requests:
            requests[req] = get_request(req, self.api)
        ...
        if handle_request == None and req_id == None:
            ...
            workspace = get_workspace(requests[req_id].workspace_id, self.api)
            ...
            set_request(req_id, requests[req_id].dicc(), self.api)
    ...
    def _workspaces(self):
        ...
        for w in user.workspaces:
            workspaces.append(get_workspace(w, self.api))
    ...
    def _edit_user(self):
        ...
        if alias != None:
            same_alias_user = get_user(alias, self.api)

        if same_alias_user != None:
            print(f"User with alias {alias} already exists")
            return

        new_user = User(alias or user.alias,
                        name or user.full_name, password or user.password)
    ...
    def _create_workspace(self):
        ...
        set_user(user.alias, user.dicc(), self.api)
        set_workspace(new_workspace.workspace_id,
                      new_workspace.dicc(), self.api)
    ...
    def _remove_workspace(self):
        ...
        user = get_user(self.logged_user, self.api)
        remove = user.remove_workspace(workspace_id, self.api)
    ...
    def _add_user(self):
        ...
        user = get_user(self.logged_user, self.api)
        user_to_add = get_user(user_alias, self.api)
        ...
        set_user(user.alias, user.dicc(), self.api)
        set_user(user_to_add.alias, user_to_add.dicc(), self.api)
        set_workspace(workspace_id, workspace.dicc(), self.api)
    ...
    def _remove_user(self):
        ...
        user = get_user(self.logged_user, self.api)
        user_to_remove = get_user(user_alias, self.api)
        workspace = get_workspace(workspace_id, self.api)
        ...
        if remove:
            set_user(user.alias, user.dicc(), self.api)
            set_user(user_to_remove.alias, user_to_remove.dicc(), self.api)
            set_workspace(workspace_id, workspace.dicc(), self.api)
    ...
    def _change_role(self):
        ...
        user_to_change = get_user(user_alias, self.api)
        workspace = get_workspace(workspace_id, self.api)
        ...
        set_user(user.alias, user.dicc(), self.api)
        set_user(user_to_change.alias, user_to_change.dicc(), self.api)
        set_workspace(workspace.workspace_id, workspace.dicc(), self.api)
    ...
    def _create_event(self):
        ...
        if users_collision != []:
            print(
                f"WARNING: User {users_collision[0]} has an event that collides with the new event.")
            while True:
        ...
        set_user(user_get.alias, user_get.dicc(), self.api)

        for u in users:
            set_user(u.alias, u.dicc(), self.api)

        set_workspace(workspace_get.workspace_id,
                      workspace_get.dicc(), self.api)
    ...
    def _remove_event(self):
        ...
        user = get_user(self.logged_user, self.api)
        event = get_event(event_id, self.api)
        workspace = get_workspace(event.workspace_id, self.api)
    ...
    def _events(self):
        ...
        for i, e in enumerate(workspace.events):
            print(f"{i+1}. {get_event(e, self.api)}")
    ...
    def _set_event(self):
        ...
        if new_event != None:
            set_event(new_event.event_id, new_event.dicc(), self.api)
    ...
    def _user_subparser(self):
        ...
        # register
        register = self.subparsers.add_parser(
            'register', help='User registration')
        register.add_argument('alias', help='Alias', type=str, default=None)
        register.add_argument(
            'full_name', help='Full name', type=str, default=None)
        register.add_argument('password', help='Password',
                              type=str, default=None)
        register.add_argument(
            'confirmation', help='Confirm password', type=str, default=None)

        # logout
        logout = self.subparsers.add_parser('logout', help='Logout')

        # inbox
        inbox = self.subparsers.add_parser(
            'inbox', help='Get inbox with notifications and requests')
        inbox.add_argument('--handle_request', choices=[
                           'accept', 'reject'], help='Accept or reject a request in inbox', type=str, default=None)
        inbox.add_argument(
            '--req_id', help='Identificator of request to handle', type=str, default=None)

        # workspaces
        workspaces = self.subparsers.add_parser(
            'workspaces', help='Get list of all workspaces of the user')

        # edit profile
        edit_user = self.subparsers.add_parser(
            'edit_profile', help='Edit user profile', )
        edit_user.add_argument(
            '--alias', help='Edit alias', type=str, default=None)
        edit_user.add_argument(
            '--name', help='Edit name', type=str, default=None)
        edit_user.add_argument(
            '--password', help='Edit password', type=str, default=None)

        # exit workspace
        exit_workspace = self.subparsers.add_parser(
            'exit_workspace', help='Exit workspace')
        exit_workspace.add_argument('workspace_id', help='if of workspace')

    def _workspaces_subparsers(self):

        # create workspace
        create_workspace = self.subparsers.add_parser(
            'create_workspace', help='Create new workspace')
        create_workspace.add_argument(
            'title', help='Title of workspace', default=None)
        create_workspace.add_argument(
            'type', choices=['f', 'h'], help='Type of workspace', default=None)

        # remove workspace
        remove_workspace = self.subparsers.add_parser(
            'remove_workspace', help='Remove workspace')
        remove_workspace.add_argument(
            'id', help="id of workspace. You can run command 'workspaces' to see id.", default=None)

        # add user
        add_user = self.subparsers.add_parser(
            'add_user', help='Add user to workspace')
        add_user.add_argument(
            'workspace_id', help='Id of workspace', default=None)
        add_user.add_argument(
            'user_alias', help='Alias of user to add', default=None)

        # remove user
        remove_user = self.subparsers.add_parser(
            'remove_user', help='Remove user from workspace')
        remove_user.add_argument(
            'workspace_id', help='Id of workspace', default=None)
        remove_user.add_argument(
            'user_alias', help='Alias of user to remove', default=None)

        # get users
        get_users = self.subparsers.add_parser(
            'get_users', help='Get users from workspace')
        get_users.add_argument(
            'workspace_id', help='id of workspace', default=None)

        # change_role
        change_role = self.subparsers.add_parser(
            'change_role', help='Change role of user in workspace')
        change_role.add_argument(
            'user_alias', help='Alias of user', default=None)
        change_role.add_argument(
            'workspace_id', help='id of workspace', default=None)

        # create event
        create_event = self.subparsers.add_parser(
            'create_event', help="Create an event in a workspace of the user")
        ...
        create_event.add_argument(
            'place', help='Place of the event', default=None)
        create_event.add_argument(
            'start_time', help='Start time', default=None)
        create_event.add_argument('end_time', help='End time', default=None)

        # remove event
        remove_event = self.subparsers.add_parser(
            'remove_event', help='Remove event from workspace')
        remove_event.add_argument('event_id', help='id of event', default=None)

        # get events
        events = self.subparsers.add_parser(
            'events', help='Get list of events of a workspace')
        events.add_argument(
            'workspace_id', help='id of workspace', default=None)
        ...
        change_workspace = self.subparsers.add_parser(
            'change_workspace_type', help='Change type of a workspace')
        change_workspace.add_argument(
            'workspace_id', help='id of workspace', default=None)
        change_workspace.add_argument(
            '--admins', help='Administrators of workspace', type=list, default=None)
